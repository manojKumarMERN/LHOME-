...
const DynamicIterableComponent: React.FC<Properties> = ({ data = [], categoryId }) => {
  ...
  const handleLike = async (id: number) => {
    ...
    try {
      ...
      const response = await AxiosService.put(`/products/wishlist/${id}`, {}, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      if (response.status === 200) {
        setRes((prevRes) => {
          const updatedRes = prevRes.map((item) =>
            item.id === id ? { ...item, liked: !item.liked } : item
          );
          return updatedRes;
        });
        toast('Product liked successfully');
      } else {
        toast('Failed to like product. Please try again.');
      }
    } catch (error) {
  ...
  const updatedData = data.map((element) => {
    ...
    if (matchingItem) {
      return { ...element, liked: matchingItem.liked };
    }
    return element;
  });
...
//           },
//         });
//         console.log(response.data.cartItems.product.id)
//         // setRes(Array.isArray(response.data?.wishlist) ? response.data.wishlist : []);
//       } catch (error) {
//         console.error('Error:', error.message);
//       }
...
//       if (response.status === 200) {
//         const updatedResponse = await AxiosService.post('/wishes', {
//           loginId: token,
//           categoryId: categoryId,
//         });
//         setRes(Array.isArray(updatedResponse.data?.wishlist) ? updatedResponse.data.wishlist : []);
//       } else {
